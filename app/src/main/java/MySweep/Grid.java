...
public class Grid extends JPanel {
    ...
    private static final Insets CellInset = new Insets(-20, -20, -20, -20);// <-- leave this alone unless you want dots
                                                                           // instead of numbers
    ...
    private class CellButton extends JButton {// these are our game board buttons. This class was created to allow the
        ...
        private Color borderColor = defaultBorderColor;// <--not defined inside class because that would mean creating a
                                                       // new color for each button, which is expensive.
        private int x, y;
        ...
        public CellButton() {// <-- -Constructor
            ...
            this.setHorizontalAlignment(SwingConstants.CENTER);// <-- they used to be in their own loop but this is the
                                                               // same thing
            this.setVerticalAlignment(SwingConstants.CENTER);// Initialize our cell button properties other than font
                                                             // size (which is done after packing layout)
    ...
    private class ScalableIcon implements Icon {// currently only used in GameOver function
        ...
        public ScalableIcon(Image originalImage) {// <-- Constructor
            this.originalIcon = new ImageIcon(originalImage);// <-- create an ImageIcon out of the icon. It is not easy
                                                             // to get the height of the Image class.
        }

        public int getIconWidth() {
            return 0;
        }// <-- it only matters that these are smaller than the buttons. If it isnt, the
         // button will get bigger.

        public int getIconHeight() {
            return 0;
        }// <-- We are implementing an interface. Other java functions use these
         // functions. One of those, is JButton.

        public void paintIcon(Component c, Graphics g, int x, int y) {// <-- This will then make the icon the size of
                                                                      // the button after.
    ...
    // --------------------getButtonAt(int x, int y) Became necessary after getting
    // rid of 2d array to reference cells by location-------------------------
    private CellButton getButtonAt(int x, int y) {
        return (CellButton) Grid.this.getComponent(y * Fieldx + x);
    }

    // -----------------------------------function for adding mouse Listener to
    // cells in Grid-------------------------------------------------
    void addCellListener(MouseListener mouseListener) {// ----------addCellListener()--------------------------------------------------
        for (int i = 0; i < Fieldx; i++) {
            for (int j = 0; j < Fieldy; j++) {
                getButtonAt(i, j).addMouseListener(mouseListener);
            } // The short time where your click wont immediately update the visuals yet after
              // board becomes visible on large board sizes
        } // ^ isnt due to this function its just because its java (tm).
    }// ----------------------------------------------------------------------------------------------------------------------------------------

    @Override
    protected void paintComponent(Graphics g) {// <--Look what you have to do to make the background GRASS without
                                               // making the components also GRASS??
        g.setColor(GRASS);
    ...
    // Functions--------------------------------------------------------------------
    // ---------Misc Public Display
    // Functions-----------------------------------------Misc Public Display
    // Functions---------------------------
    long getTime() {
        return (answers.isFirstClick()) ? (-1) : answers.getTime();
    }// <-- i passed this through here to keep surface area small for brain
    ...
    void toggleQuestionMarks() {// <-- toggles the ability to mark cells with a ? on and off (i find it annoying
                                // so it starts turned off)
            ...
            for (int x = 0; x < Fieldx; x++) {
                for (int y = 0; y < Fieldy; y++) {
                    if (answers.isQuestionable(x, y)) {
                        ...
                        getButtonAt(x, y)
                                .setForeground((MineSweeper.isDarkMode()) ? DarkModeTextColor : LightModeTextColor);
        ...
        for (int x = 0; x < Fieldx; x++) {
            for (int y = 0; y < Fieldy; y++) {
                if (!(answers.exploded(x, y) || (answers.checked(x, y) && answers.adjCount(x, y) == 0))) {
                    ...
                    if (answers.isBomb(x, y) && answers.isGameOver()) {// <-- literally just stick this section in
                                                                       // there.
    ...
    void ResetBoard() {// <-- for when you want new but not like, new new. Just like, sorta new.
                       // Refreshed.
        BombsFound = 0;
        livesLeft = lives;
        GameOverMessageIndex = -1;
        wonValue = -1;
        answers = new Minefield(Fieldx, Fieldy, bombCount);// <-- get a new minefield, thus resetting the timer and
                                                           // everything else
        for (int i = 0; i < Fieldx; i++) {
        ...
        Grid.this.repaint();// repaint so that it actually repaints at a time that makes sense rather than
                            // just like... later.
    }

    int[] doZoom(int rotation, int mouseX1, int mouseY1) {// it makes the cells bigger. The main window is in a scroll
                                                          // pane. does not set font
    ...
    // --------Click Handler--------------Click Handler--------------Click
    // Handler--------------Click Handler--------------Click Handler------
    ...
    // -----------END OF PUBLIC FUNCTIONS------------------------END OF PUBLIC
    // FUNCTIONS------------------------END OF PUBLIC
    // FUNCTIONS-----------------------
    // -----------MAIN CELL CLICKED FUNCTIONS-----------------MAIN CELL CLICKED
    // FUNCTIONS----------MAIN CELL CLICKED
    // FUNCTIONS--------------------------------
    private void regularClick(CellButton current) {// ---------regularClick()--------regularClick()--------regularClick()--------regularClick()----
        ...
        if (!answers.isGameOver()) {
            if (answers.isFirstClick()) {
                answers.reset(xValue, yValue);// if possible, answers.adjCount(clickedX,clickedY) == 0 on the first
                                              // click. also initializes field data.
            ...
            if (!answers.exploded(xValue, yValue) && !answers.checked(xValue, yValue) && !answers.marked(xValue, yValue)
                ...
                if (answers.isBomb(xValue, yValue)) {// was it a bomb? ^was it a clickable cell?
                    ...
                    boolean[][] prechekd = new boolean[Fieldx][Fieldy];// saving current checks so we know which ones
                                                                       // fillzeroes flipped
                    ...
                    markLonelyBombs(prechekd);// <-- mark the bombs with all neighbors 1 that were completely revealed
                                              // by fillzeroes
                    BombsFound = answers.cellsMarked() + answers.cellsExploded();
                }
            }
    ...
    private void playChord(CellButton current) {// Chord reveals all cells around a number, but you need to mark
                                                // correctly or it hurts.
        int a = current.getXcoord();// ^ did you know about this? I didnt either... I checked the minesweeper rules
                                    // again...
                        ...
                        if (!answers.exploded(i, j) && !answers.marked(i, j)) {
                            if (answers.isBomb(i, j)) {
                                ...
                                if (answers.adjCount(i, j) != 0) {// adjCount>0
                                    ...
                                    boolean[][] prechekd = new boolean[Fieldx][Fieldy];// saving current checks so we
                                                                                       // know which ones fillzeroes
                                                                                       // flipped
                                    for (int x = 0; x < Fieldx; x++) {
                                        for (int y = 0; y < Fieldy; y++) {
                                    ...
                                    markLonelyBombs(prechekd);// <-- mark the bombs with all neighbors 1 that were
                                                              // completely revealed by fillzeroes
                                    BombsFound = answers.cellsMarked() + answers.cellsExploded();
                                }
                            }
                        }
    ...
    }// --------------end of main cell clicked functions----Helper functions they
     // referenced below---------------------------------------------------------
     // ------------------------------------------fillZeroes()-------------------------------------------------

    private void fillZeroes(int xValue, int yValue) {// -----------fillZeroes()------------------------------------------------------------------
        Stack<Integer> stack = new Stack<>();// a stack must be used instead of recursion because large board sizes
                                             // cause stack overflow.
        stack.push(yValue * Fieldx + xValue);// make single value out of x and y (in this order because thats the same
                                             // as getButtonAt)
        while (!stack.isEmpty()) {
            ...
            if (!answers.checked(x, y)) {// since this will unfortunately branch and call multiple times per 0 cell,
                ...
                ((CellButton) Grid.this.getComponent(position)).setText(String.valueOf(answers.adjCount(x, y)));// direct
                                                                                                                // from
                                                                                                                // position
                                                                                                                // because
                                                                                                                // otherwise
                ((CellButton) Grid.this.getComponent(position)).setBackground(GRASS); // you do the multiplication twice
                                                                                      // unnecessarily
    ...
    // -----------------------------------------------------markLonelyBombs()----------------------------------------------------------------
    private void markLonelyBombs(boolean[][] prechekd) {// I didnt want to click on all the lone bombs on big boards in
                                                        // order to
                ...
                if (answers.isBomb(j, k)) {// <-- check for "is bomb"
                    ...
                    for (int l = j - 1; l <= j + 1; l++) {// check neighbors
                        for (int m = k - 1; m <= k + 1; m++) {
                            if (!(l < 0 || m < 0 || l >= Fieldx || m >= Fieldy || (l == j && m == k))) {// <-- it was
                                                                                                        // valid cell
    ...
    private void GameOver(boolean won) {// reveals bombs on board then passes the work to ScoresFileIO
        ...
        MessageIndex = ScoresFileIO.updateScoreEntry(won, answers.getTime(), answers.cellsExploded(), Fieldx, Fieldy,
                bombCount, lives);
